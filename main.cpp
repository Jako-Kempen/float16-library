...
/*void convertFloatToFloat16(float fValue, float16* newValue)
{
	...
	while (check == 0)
	{
		...
		if ((increment < -130) || (increment > 130))
		{
			std::cout << "Endlees loop detected" << increment << ": " << fValue << endl;
			exit;
		}
		
	}
	
	newValue->Set_Exp(128+increment);
	
	//remove the implicit 1
	fValue--;
	uint8_t tempMantissa = 0x00;//Initialise the temporary mantissa as 0x00;
	
	//Generate mantissa
	if ((fValue - 0.5) >= 0)
	{
		tempMantissa = tempMantissa | 0b01000000; //0b01000000
		fValue -= 0.5;
		std::cout << "Current fValue: " << fValue << " Current mantissa: " << static_cast<int>(tempMantissa) << endl;
	}//check 2^-1
	if ((fValue - 0.25) >= 0)
	{
		tempMantissa = tempMantissa | 0b00100000; //0b00100000
		fValue -= 0.25;
		std::cout << "Current fValue: " << fValue << " Current mantissa: " << static_cast<int>(tempMantissa) << endl;
	}//check 2^-2
	if ((fValue - 0.125) >= 0)
	{
		tempMantissa = tempMantissa | 0b00010000; //0b00010000
		fValue -= 0.125;
		std::cout << "Current fValue: " << fValue << " Current mantissa: " << static_cast<int>(tempMantissa) << endl;
	}//check 2^-3
	if ((fValue - 0.125) >= 0)
	{
		tempMantissa = tempMantissa | 0b00001000; //0b00001000
		fValue -= 0.125;
		std::cout << "Current fValue: " << fValue << " Current mantissa: " << static_cast<int>(tempMantissa) << endl;
	}//check 2^-3
	if ((fValue - 0.0625) >= 0)
	{
		tempMantissa = tempMantissa | 0b00000100; //0b00000100
		fValue -= 0.0625;
		std::cout << "Current fValue: " << fValue << " Current mantissa: " << static_cast<int>(tempMantissa) << endl;
	}//check 2^-4
	if ((fValue - 0.03125) >= 0)
	{
		tempMantissa = tempMantissa | 0b00000010; //0b00000010
		fValue -= 0.03125;
		std::cout << "Current fValue: " << fValue << " Current mantissa: " << static_cast<int>(tempMantissa) << endl;
	}//check 2^-5
	if ((fValue - 0.015625) >= 0)
	{
		tempMantissa = tempMantissa | 0b00000001; //0b00000001
		fValue -= 0.015625;
		std::cout << "Current fValue: " << fValue << " Current mantissa: " << static_cast<int>(tempMantissa) << endl;
	}//check 2^-6
	...
	if (sign == 1)
	{
		tempMantissa = tempMantissa | 0b10000000; //0b10000000
	}
